...
const DetailedJobPosting = () => {
  ...
  const handleApprove = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:3001/api/jobpost/approve/${id}`, {
        method: "PUT",
      });
      if (!res.ok) {
  ...
  const handleReject = async (id: string) => {
    try {
      const res = await fetch(`http://localhost:3001/api/jobpost/reject/${id}`, {
        method: "PUT",
      });
      if (!res.ok) {
        throw new Error(`HTTP error! status: ${res.status}`);
      ...
      <div id={formData?._id} className="col-span-3">
        <div className="rounded-md bg-white p-6">
          <Badge
            variant={
              formData.statusApproval === true && formData.statusSeeking === true
                ? "default"
                : formData.statusApproval === false && formData.statusSeeking === false
                  ? "destructive"
                  : "secondary"
            }
          >
          ...
          <div className="mt-8">
            ...
            <div className="flex items-center">
              ...
              <span>
                {formData?.companyAddress || "Chưa cập nhật"}
              </span>
            </div>
          </div>
        </div>
        <div className="space-x-2">
          <Button
            ...
            disabled={formData.statusSeeking == false && formData.statusApproval == false}
          >
          ...
          <Button
            ...
            disabled={formData.statusSeeking == true && formData.statusApproval == true}
          >
