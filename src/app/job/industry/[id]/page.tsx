          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="line-clamp-1 text-gray-600">{job.companyName}</p>
              </div>
...
export default function JobListings() {
  ...
  useEffect(() => {
    const fetchJobPosts = async () => {
      try {
        const industry = Array.isArray(params.id)
          ? decodeURIComponent(params.id[0]) // Nếu là mảng, lấy phần tử đầu tiên
          : decodeURIComponent(params.id); // Nếu không, sử dụng trực tiếp

        const res = await fetch(
          `http://localhost:3001/api/job-info/jobs/industry/${industry}`
        );
          ...
          <div>
            <h1 className="text-3xl font-bold text-blue-900">
              ...
              <span className="text-orange-300">
                {jobPosts[0]?.jobIndustry}
              </span>
            </h1>
            <p className="mt-2 text-gray-600">
              Khám phá những công việc nổi bật trong lĩnh vực{" "}
              {jobPosts[0]?.jobIndustry}, được cập nhật hàng ngày để mang đến
              cho bạn các cơ hội nghề nghiệp phù hợp nhất, giúp bạn phát triển
              sự nghiệp.
            </p>
          </div>
