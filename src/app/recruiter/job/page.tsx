                ...
                <div className="container py-6">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex space-x-2">
                            ...
                            <button className="border border-gray-300 px-4 py-2 rounded"><FaSliders className="fas fa-sliders-h" /></button>
                        </div>
                    </div>
                        ...
                        {formData?.map((job: any) => (
                            <div className="border-t border-gray-300 py-4">
                                <div className="grid grid-cols-4">
                                    <div>
                                        <div className="text-blue-500 font-semibold">{job?.jobTitle}</div>
                                        <div className="text-gray-500">{job?._id}</div>
                                        <div className="text-gray-500">{job?.jobLocation}</div>
                                        <div className="text-gray-500">Đăng vào {new Date(
                                            job?.datePosted
                                        ...
                                        <div className="flex space-x-2 mt-2">
                                            <Link href={`/recruiter/edit-job/${job?._id}`}>
                                                <FaPencilAlt className="cursor-pointer fas fa-pencil-alt" />
                                            </Link>
                                            <i className="fas fa-copy" />
                                            <FaEye className="cursor-pointer fas fa-eye" />
                                            <i className="fas fa-users" />
                                            <FaTrash onClick={() => handleDeleteJob(job?._id)} className="cursor-pointer fas fa-trash" />
                                        </div>
                                    </div>
                                    <div>
                                    ...
                                    <div className="text-blue-500">{job?.statusApproval ? "Đã duyệt" : "Chờ duyệt"}</div>

                                    <div>
                                        {job?.statusSeeking ? (

                                            <button className="cursor-pointer border border-red-500 text-red-500 px-4 py-2 rounded"
                                                onClick={() => handleCancel(job?._id)}>Ngưng nhận hồ sơ
                                            </button>
                                        ) : (
                                            <button disabled className="cursor-pointer border border-gray-500 text-gray-500 px-4 py-2 rounded"
                                                onClick={() => handleCancel(job?._id)}>Ngưng nhận hồ sơ
                                            </button>
                                        )}
                                    </div>
