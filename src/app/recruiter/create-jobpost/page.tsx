...
const CreateJobpost = () => {
  ...
  const handleSubmit = (event: any) => {
    ...
    if (nullFields.length > 0) {
      ...
      if (labels.length > 1) {
        toast(`Vui lòng không bỏ trống: ${labels.join(", ")}`);
      }
  ...
  const handleInputChangeKeyWord = async (e: any) => {
    ...
    debounceTimeout = setTimeout(async () => {
      if (e.target.value.length > 1) {
        const message = `Gợi ý cho tôi 20 từ khóa về kỹ năng và nghề nghiệp có chứa từ ${e.target.value} thuộc lĩnh vực ${formData?.jobInformation?.jobField || formData?.jobInformation?.jobIndustry || ""}`;
        try {
          const result = await chatSession.sendMessage(message);
          const data = result?.response?.text()
          // const data = ["Java Developer", "Software Engineer", "Backend Developer", "Full Stack Developer", "Android Developer", "Java Architect", "Spring Boot Developer", "Microservices Developer", "Java Web Developer", "Big Data Engineer"]
          console.log(data)
          setSuggestions([...data])
        } catch (error) {
