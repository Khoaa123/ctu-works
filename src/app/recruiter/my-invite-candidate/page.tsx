...
const MyInviteCandidate = () => {
  ...
  const handleSearch = (e: any) => {
    const filtered = myInvite.filter(
      (apply) =>
        apply.userName.toLowerCase().includes(e.target.value.toLowerCase()) ||
        apply.userEmail.toLowerCase().includes(e.target.value.toLowerCase()) ||
        apply.userPhone
          .toString()
          .toLowerCase()
  ...
  const setFillter = (status: any) => {
    if (status === "all") {
      ...
      const filtered = myInvite.filter((item) => item.status === status);
      setFilteredApplies(filtered);
      setFilter(status);
    }
  };
    ...
    <div>
      ...
      <div className="p-6">
        <div className="rounded-lg bg-white p-6 shadow-md">
          <h1 className="mb-2 text-2xl font-bold">Lời mời ứng tuyển</h1>
          ...
          <div className="mb-4 border-b border-gray-200">
            <div className="flex space-x-4">
              ...
              <button
                ...
                className={buttonStyles("Chờ phản hồi")}
              >
                Chờ phản hồi
              </button>
              <button
                onClick={() => setFillter("Đã chấp nhận")}
                className={buttonStyles("Đã chấp nhận")}
              >
                Đã chấp nhận
              </button>
              <button
                onClick={() => setFillter("Đã từ chối")}
                className={buttonStyles("Đã từ chối")}
              >
                Đã từ chối
              </button>
              <button
                onClick={() => setFillter("Đã hết hạn")}
                className={buttonStyles("Đã hết hạn")}
              >
                Đã hết hạn
              </button>
            </div>
          ...
          <div className="relative overflow-x-auto">
            <table className="min-w-full bg-white">
              <thead>
                <tr>
                  <th className="bg-gray-100 px-4 py-2 text-left text-gray-600">
                    Ứng viên <i className="fas fa-sort-down"></i>
                  </th>
                  <th className="bg-gray-100 px-4 py-2 text-left text-gray-600">
                    Công việc <i className="fas fa-filter"></i>
                  </th>
                  <th className="bg-gray-100 px-4 py-2 text-left text-gray-600">
                    Ngày gửi lời mời <i className="fas fa-sort-down"></i>
                  </th>
                  <th className="bg-gray-100 px-4 py-2 text-left text-gray-600">
                    Trạng thái
                  </th>
                  ...
                  <tr>
                    <td colSpan={5} className="py-8 text-center">
                      ...
                      <div className="flex flex-col items-center">
                        <img
                          src="https://employer.vietnamworks.com/v2/candidate/manage-by-folders-and-tags/static/media/empty-state.cb5e9abb7fb062c980d12e05452221ef.svg"
